...
@pytest.mark.asyncio
async def test_user_sees_only_active_products(client, session):
...
@pytest.mark.asyncio
async def test_pagination_works_on_product_list(client, session):
    ...
    response = await client.get("/products/?limit=2&offset=1", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["limit"] == 2
...
@pytest.mark.asyncio
async def test_search_filters_by_name_or_sku(client, session):
    ...
    resp = await client.get("/products/?search=alpha", headers=headers)
    assert resp.status_code == 200
    results = resp.json()["data"]
    assert len(results) == 1
    assert results[0]["short_name"] == "Alpha Product"

    resp = await client.get("/products/?search=zzz", headers=headers)
    assert resp.status_code == 200
    results = resp.json()["data"]
    assert len(results) == 1
...
@pytest.mark.asyncio
async def test_filter_by_category_id(client, session):
    ...
    resp = await client.get(f"/products/?category_id={cat1.id}", headers=headers)
...
@pytest.mark.asyncio
async def test_admin_can_filter_by_is_active(client, session):
    ...
    resp = await client.get("/products/?is_active=true", headers=headers)
    assert resp.status_code == 200
    skus = [p["sku"] for p in resp.json()["data"]]
    assert "ACTIVE01" in skus
    assert "INACTIVE01" not in skus

    resp = await client.get("/products/?is_active=false", headers=headers)
    assert resp.status_code == 200
    skus = [p["sku"] for p in resp.json()["data"]]
...
@pytest.mark.asyncio
async def test_user_cannot_filter_by_is_active(client, session):
...
# [X] GET    /products/{id}
@pytest.mark.asyncio
async def test_admin_can_view_any_product(client, session):
    ...
    response = await client.get(f"/products/{product.id}", headers=headers)
    assert response.status_code == 200
    assert response.json()["sku"] == "ADM001"


@pytest.mark.asyncio
async def test_user_can_view_active_product(client, session):
    ...
    user = await create_user_in_db(session, "user", "user@x.com", "pass", is_active=True)
    token = await get_token_for_user(client, user.email, "pass")
    headers = get_auth_headers(token)
...
@pytest.mark.asyncio
async def test_user_cannot_view_inactive_product(client, session):
    ...
    user = await create_user_in_db(session, "user", "user@z.com", "pass", is_active=True)
    token = await get_token_for_user(client, user.email, "pass")
    headers = get_auth_headers(token)
...
@pytest.mark.asyncio
async def test_get_nonexistent_product_returns_404(client, session):
    """Ensure getting a non-existent product returns 404"""
    headers, _ = await get_admin_headers(client, session)
    response = await client.get("/products/9999", headers=headers)
...
@pytest.mark.asyncio
async def test_admin_cannot_create_duplicate_sku(client, session):
...
@pytest.mark.asyncio
async def test_admin_cannot_create_product_with_invalid_category(client, session):
...
@pytest.mark.asyncio
async def test_user_cannot_create_product(client, session):
    """Ensure regular users cannot create products (forbidden)"""
    user = await create_user_in_db(session, "Regular", "user@x.com", "pass", is_active=True)
    token = await get_token_for_user(client, user.email, "pass")
    headers = get_auth_headers(token)
...
@pytest.mark.asyncio
async def test_admin_cannot_create_product_with_invalid_data(client, session):
    ...
    response = await client.post("/products/", json=bad_data, headers=headers)
    assert response.status_code == 422

# [X] PUT    /products/{id}
@pytest.mark.asyncio
async def test_user_cannot_update_product_fields(client, session):
    ...
    user = await create_user_in_db(session, "User", "u@x.com", "pass", is_active=True)
    token = await get_token_for_user(client, user.email, "pass")
    headers = get_auth_headers(token)
    ...
    response = await client.put(f"/products/{product.id}", json=update_data, headers=headers)
    assert response.status_code == 403
    assert "permission" in response.json()["detail"].lower()


@pytest.mark.asyncio
async def test_admin_can_update_all_fields_in_product(client, session):
    ...
    response = await client.put(f"/products/{product.id}", json=update, headers=headers)
    assert response.status_code == 200
    assert response.json()["is_active"] is False


@pytest.mark.asyncio
async def test_admin_can_reactivate_product(client, session):
    ...
    headers, _ = await get_admin_headers(client, session)
    response = await client.put(f"/products/{product.id}", json={"is_active": True}, headers=headers)
...
@pytest.mark.asyncio
async def test_update_product_with_existing_sku_fails(client, session):
    ...
    headers, _ = await get_admin_headers(client, session)
    response = await client.put(f"/products/{prod2.id}", json={"sku": "ABC123"}, headers=headers)
    assert response.status_code == 400
    assert "sku" in response.json()["detail"].lower()


@pytest.mark.asyncio
async def test_update_nonexistent_product_returns_404(client, session):
    """Ensure updating non-existent product returns 404"""
    headers, _ = await get_admin_headers(client, session)
    response = await client.put("/products/9999", json={"short_name": "Ghost"}, headers=headers)
    assert response.status_code == 404
    assert "not found" in response.json()["detail"].lower()

@pytest.mark.asyncio
async def test_admin_cannot_deactivate_product_with_stock(client, session):
    ...
    headers, _ = await get_admin_headers(client, session)
    response = await client.put(f"/products/{fake_prod.id}", json={"is_active": False}, headers=headers)

    await session.refresh(fake_prod)
    assert response.status_code == 400
    assert fake_prod.is_active == True

@pytest.mark.asyncio
async def test_admin_can_deactivate_products_without_stock(client, session):
...
@pytest.mark.asyncio
async def test_admin_can_bulk_deactivate_products_without_stock(client, session):
...
async def test_admin_cannot_deactivate_product_with_stock(client, session):
    ...
    p1_updated = await session.get(Product, p1.id)
    assert p1_updated.is_active is True
    p2_updated = await session.get(Product, p2.id)
    assert p2_updated.is_active is False


@pytest.mark.asyncio
async def test_regular_user_cannot_bulk_update_products(client, session):
...
@pytest.mark.asyncio
async def test_admin_can_delete_product_without_movements(client, session):
...
@pytest.mark.asyncio
async def test_admin_cannot_delete_product_with_movements(client, session):
...
@pytest.mark.asyncio
async def test_regular_user_cannot_delete_product(client, session):
...
@pytest.mark.asyncio
async def test_delete_nonexistent_product_returns_404(client, session):
    """Ensure deleting a non-existent product returns 404"""
    headers, _ = await get_admin_headers(client, session)
    response = await client.delete("/products/9999", headers=headers)
    assert response.status_code == 404
    assert "not found" in response.json()["detail"].lower()
